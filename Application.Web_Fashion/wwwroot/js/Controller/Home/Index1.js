...
app.controller('HomeCtrl', ['$rootScope', '$scope', '$http', '$filter', '$location', 'Enum', 'homePageService', function ($rootScope, $scope, $http, $filter, $location, Enum, homePageService) {
    ...
    //        var plus_attr = 'id="btnPlus_ci_' + productList[i].Id + '" productId="' + productList[i].Id + '"';
    //        var minus_attr = 'id="btnMinus_ci_' + productList[i].Id + '" productId="' + productList[i].Id + '"';
    //        var qty_attr = 'id="txtQty_ci_' + productList[i].Id + '" productId="' + productList[i].Id + '"';
        ...
        homePageService.getCategoryList()
            .success(function (cats) {
                ...
                for (var i = 0; i < cats.length; i++) {
                    html += '<li>' +
                                '<a style="display: block; position: relative;" href="/Product/Search?cat=' + cats[i].Id + '&catName=' + cats[i].Name + ' "><img src="/Photos/Categories/Icons/'+ cats[i].IconName +'" style="width:16px; margin-right:5px; position:relative; top:-2px;">' + cats[i].Name + '</a>' + '<img src="/Images/right-arrow.png" style="height: 15px; margin-left: 5px; float:right; position:relative;top:-16px;">' +
                            '</li>';
                }
            ...
            .success(function (productList) {
                ...
                for (var i = 0; i < productList.length; i++) {
                    ...
                    $('#homepage-container-popular').append(
                        '<div class="grid-item"> ' +
                        '<div class="div-item-container clearfix">' +
                        '<a class="item-link-container clearfix" href="/Product/Details?id=' + productList[i].Id + '&t=' + productList[i].TitleSEO + '"> ' +
                                    '<div class="grid-item-image"> ' +
                                        '<img src="' + productList[i].PrimaryImageName + '" /> ' +
                                    '</div> ' +
                                    '<div class="grid-item-info"> ' +
    ...
    $('#category-wise-container').on('click', '.btn-plus', function (event) {
        var productId = $(this).attr('productId');

        var qty = $('#txtQty_ci_' + productId).val();
        var newQty = parseInt(qty, 10) + 1;

        $('#txtQty_ci_' + productId).val(newQty);
        event.preventDefault();
    });

    $('#category-wise-container').on('click', '.btn-minus', function (event) {
        var productId = $(this).attr('productId');
        var qty = $('#txtQty_ci_' + productId).val();
    ...
    $('#category-wise-container').on('click', '.home-add-to-cart', function (event) {
        var productId = $(this).attr('productId');
        var name = $(this).attr('name');
        var price = $(this).attr('price');
        var imageUrl = $(this).attr('imageUrl');
